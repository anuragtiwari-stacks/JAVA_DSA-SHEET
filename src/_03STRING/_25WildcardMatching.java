package _03STRING;

public class _25WildcardMatching
{

    // Main matching function (recursive)
    public static boolean match(String wild, String pattern)
    {
        return matchHelper(wild.toCharArray(), pattern.toCharArray(), 0, 0);
    }

    // Helper function with char arrays and indices
    private static boolean matchHelper(char[] wild, char[] pattern, int i, int j)
    {
        // If both strings are fully traversed
        if (i == wild.length && j == pattern.length)
        {
            return true;
        }

        // If wild has '*' but no more characters in pattern
        if (i < wild.length && wild[i] == '*' && i + 1 < wild.length && j == pattern.length)
        {
            return false;
        }

        // If characters match or wild has '?'
        if (i < wild.length && j < pattern.length &&
                (wild[i] == '?' || wild[i] == pattern[j]))
        {
            return matchHelper(wild, pattern, i + 1, j + 1);
        }

        // If wild has '*', try two possibilities:
        // 1. '*' matches zero characters (i + 1, j)
        // 2. '*' matches one character from pattern (i, j + 1)
        if (i < wild.length && wild[i] == '*')
        {
            return matchHelper(wild, pattern, i + 1, j) ||
                    (j < pattern.length && matchHelper(wild, pattern, i, j + 1));
        }

        // All other cases - mismatch
        return false;
    }

    // Main method for testing
    public static void main(String[] args)
    {
        String wild = "a*b?d";
        String pattern = "aXYbcd";

        if (match(wild, pattern))
        {
            System.out.println("Yes, the strings match.");
        }
        else
        {
            System.out.println("No, the strings do not match.");
        }
    }
}

/*
How the code works:
We have two strings:

1-wild (may contain * and ?)
2-pattern (no wildcards)

We use a recursive helper function
matchHelper(wild, pattern, i, j) to check if wild[i..end] matches pattern[j..end].
*/


/*
-------------------- COMPLETE DRY RUN --------------------

Input:
wild    = "a*b?d"
pattern = "aXYbcd"

We start with:
matchHelper(wild, pattern, i = 0, j = 0)

---------------------------------------------------------
Step 1:
i = 0, j = 0
wild[0] = 'a'
pattern[0] = 'a'
Characters match
→ move both forward

Call:
matchHelper(i = 1, j = 1)

---------------------------------------------------------
Step 2:
i = 1, j = 1
wild[1] = '*'
'*' can match:
1) zero characters
2) one or more characters

---------------------------------------------------------
Try Case 1: '*' matches ZERO characters

Call:
matchHelper(i = 2, j = 1)

i = 2, j = 1
wild[2] = 'b'
pattern[1] = 'X'
Mismatch ❌
→ return false

---------------------------------------------------------
Try Case 2: '*' matches ONE character

Call:
matchHelper(i = 1, j = 2)

i = 1, j = 2
wild[1] = '*'
pattern[2] = 'Y'

Again '*' found, try both cases

---------------------------------------------------------
Case 2.1: '*' matches ZERO characters

Call:
matchHelper(i = 2, j = 2)

wild[2] = 'b'
pattern[2] = 'Y'
Mismatch ❌
→ return false

---------------------------------------------------------
Case 2.2: '*' matches ONE character

Call:
matchHelper(i = 1, j = 3)

wild[1] = '*'
pattern[3] = 'b'

---------------------------------------------------------
Now try ZERO match again:

Call:
matchHelper(i = 2, j = 3)

wild[2] = 'b'
pattern[3] = 'b'
Characters match ✔

---------------------------------------------------------
Step 3:
Move both forward

Call:
matchHelper(i = 3, j = 4)

wild[3] = '?'
pattern[4] = 'c'
'?' matches any one character ✔

---------------------------------------------------------
Step 4:
Move both forward

Call:
matchHelper(i = 4, j = 5)

wild[4] = 'd'
pattern[5] = 'd'
Characters match ✔

---------------------------------------------------------
Step 5:
Move both forward

Call:
matchHelper(i = 5, j = 6)

i == wild.length
j == pattern.length
Both strings completely matched ✔

---------------------------------------------------------
Return TRUE

Final Result:
The wild string matches the pattern
---------------------------------------------------------
*/
